...
public class Rectangle extends GuiElement {
    ...
    @Getter
    @Setter
    private int outlineLeft = 0;
    @Getter
    @Setter
    private int outlineTop = 0;
    @Getter
    @Setter
    private int outlineRight = 0;
    @Getter
    @Setter
    private int outlineBottom = 0;
    ...
    @Getter
    private float xGap = 1f;
    @Getter
    private float yGap = 1f;
    ...
    public Rectangle size(int width, int height) {
        if (this.x2 != -1) {
            Evie.error("[framework-" + this.name + "] Object controlled by points, size has been ignored!");
            return this;
        }
        this.x2 = x1 + width;
        this.y2 = y1 + height;
    ...
    public Rectangle build() {
        ...
        if (this.outlineColor == null && (this.outlineBottom > 0 || this.outlineTop > 0 || this.outlineLeft > 0 || this.outlineRight > 0)) {
